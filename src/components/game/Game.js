...
class Game extends Component {
  ...
  onCellClick(i) {
    ...
    var nextBoardState =
      this.state.boardState.substring(0, i) +
      cellValue +
      this.state.boardState.substring(i + 1);
    const params = {
      gameIndex: this.state.gameIndex,
  ...
  onGameEndUpdate(payload) {
    const wonGame = payload.winnerUsername === this.props.username;
    const tiedGame = payload.winnerUsername === NO_WINNER;
    if (tiedGame) {
      this.props.onGameEnded(EndGameState.TIED);
    } else if (wonGame) {
      this.props.onGameEnded(EndGameState.WON);
    } else {
      this.props.onGameEnded(EndGameState.LOST);
    }
  }
    ...
    return (
      <div>
        <p className="Regular-Text User-Label">
          ...
          <br />
          Max Bidding Power: {this.state.maxBidAmt}
        </p>
        <div className="Game">
          <Board
        ...
        <p className="Regular-Text Opponent-Label">
          ...
          <br />
          Status: {this.getOpponentStatus()}
          <br />
          Max Bidding Power: {200 - this.state.maxBidAmt}
        </p>
      </div>
    );
  }
